...
class IonicStartPanel {
    ...
    static init(extensionUri, path, context) {
        if (IonicStartPanel.currentPanel) {
            ...
            'New', vscode_1.ViewColumn.One, {
                ...
                localResourceRoots: [
                    ...
                    vscode_1.Uri.joinPath(extensionUri, 'ionic-start', 'build'),
                ],
            });
        ...
        webview.onDidReceiveMessage(async (message) => {
            ...
            switch (command) {
                ...
                case MessageType.chooseFolder: {
                    const paths = await vscode_1.window.showOpenDialog({ defaultUri: vscode_1.Uri.parse(getProjectsFolder()), canSelectFolders: true, canSelectFiles: false, canSelectMany: false });
...
async function createProject(project, webview, panel) {
    ...
    const cmds = [];
    cmds.push(`npx ionic start '${name}' ${project.template} --type=${project.type} --capacitor --package-id=${packageId} --no-git`);
...
//# sourceMappingURL=ionic-start2.js.map